...
- Fetch the target URL using `"$HOME/.claude/skills/seo/scripts/claude-seo" run render_page.py "<url>" --mode auto --json` (SPA-aware SSRF-protected renderer)
- Parse with `"$HOME/.claude/skills/seo/scripts/claude-seo" run parse_html.py --url "<url>"` to extract SEO elements
...
- [ ] URL was fetched via `"$HOME/.claude/skills/seo/scripts/claude-seo" run render_page.py --mode auto` (not raw curl)
