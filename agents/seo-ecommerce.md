...
2. Analyze product pages with `"$HOME/.claude/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto` and `"$HOME/.claude/skills/seo/scripts/claude-seo" run parse_html.py <URL>`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `"$HOME/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
"$HOME/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check <endpoint>
```
