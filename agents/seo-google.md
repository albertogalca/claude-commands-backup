---
name: seo-google
description: Google SEO API analyst. Fetches CWV field data via CrUX, indexation status via GSC, and site traffic via the Seline MCP server for enriched audit data.
model: sonnet
maxTurns: 35
tools: Read, Bash, Write, Glob, Grep, mcp__seline__seline_list_projects, mcp__seline__seline_get_data, mcp__seline__seline_get_visit_metrics, mcp__seline__seline_get_pages  # Write needed for report/data file output; Seline is the analytics source
---
...
2. Determine tier (0 = API key, 1 = + service account, 3 = + Google Ads token)
...
### Tier 0 (API Key Only)
- Run PSI + CrUX on homepage: `"$HOME/.claude/skills/seo/scripts/claude-seo" run pagespeed_check.py <url> --json`
- Run CrUX History for origin: `"$HOME/.claude/skills/seo/scripts/claude-seo" run crux_history.py <origin> --origin --json`
- Report CWV field data with traffic-light ratings

### Tier 1 (+ Service Account)
- All Tier 0 checks
- GSC top queries/pages (28 days): `"$HOME/.claude/skills/seo/scripts/claude-seo" run gsc_query.py --property <prop> --json`
...
- URL Inspection on homepage + key pages: `"$HOME/.claude/skills/seo/scripts/claude-seo" run gsc_inspect.py <url> --json`
- GSC sitemap status: `"$HOME/.claude/skills/seo/scripts/claude-seo" run gsc_query.py sitemaps --property <prop> --json`

### Analytics (Seline MCP, no Google credential needed)
- Resolve the project with `seline_list_projects`, then pass its `projectId`
- Traffic (30 days): `seline_get_visit_metrics` with `period: "30d"`, `interval: "1 day"`
- Top landing pages: `seline_get_pages` with `type: "entry"`, `period: "30d"`
- Organic slice: repeat both with
  `filters: { referrer: "contains:google;contains:bing;contains:duckduckgo;contains:ecosia;contains:brave;contains:yahoo" }`
  and label it a referrer approximation, not a modelled channel
- Never call GA4 and never ask for a GA4 property id
...
- Include data freshness notes (CrUX: 28-day rolling, GSC: 2-3 day lag, Seline: real time)
...
- `output_dir/findings/google.md`: PSI, CrUX, GSC, URL Inspection, Seline traffic, and credential-tier findings
- Structured JSON-compatible findings for `audit-data.json` under the Google SEO Data category
- Generated PDF/HTML/XLSX reports under `output_dir/` with `"$HOME/.claude/skills/seo/scripts/claude-seo" run google_report.py --output-dir "$output_dir"`
