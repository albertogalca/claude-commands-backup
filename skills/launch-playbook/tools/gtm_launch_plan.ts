 ...
 *
 * Bundled tool for the `launch-playbook` skill: it activates only while the skill
 * is loaded and runs in the skill sandbox.
 */
...
const TIER_PHASES = (tier: string): Record<string, string[]> => {
  const base = {
    pre_launch: ["Lock positioning & messaging (run the positioning-sprint skill)", "Finalize asset list & owners", "Brief sales/CS", "Stage assets & schedule"],
...
// ponytail: CLI entrypoint so this runs outside Vellum — `deno run <file> '<json>'`
if (import.meta.main) {
  const input = JSON.parse(Deno.args[0] ?? "{}");
  console.log((await run(input, {} as ToolContext)).content);
}
