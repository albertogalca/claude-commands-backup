...
If messaging isn't crisp, run the **positioning-sprint** skill (it carries `positioning_brief`) so the launch narrative is sharp before assets are produced.
...
Lock the success metrics from the brief into specific targets (e.g. "300 sourced MQLs in 30 days"). Load the **demand-plan** skill and use its **`funnel_math`** to translate any pipeline goal into required MQLs.
...
## Running `gtm_launch_plan` in Claude Code

This skill's tool is not registered as an assistant tool here. Run it with Bash:

```bash
deno run --no-check --quiet {baseDir}/tools/gtm_launch_plan.ts '<json-input>'
```

The JSON input fields are documented in `{baseDir}/TOOLS.json`. If `deno` is
unavailable, say so — do not invent the tool's output.
