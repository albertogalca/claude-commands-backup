...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), search performance (spawn when Google API credentials detected via `"$HOME/.claude/skills/seo/scripts/claude-seo" run google_auth.py --check`)
...
If Google API credentials are configured (`"$HOME/.claude/skills/seo/scripts/claude-seo" run google_auth.py --check`), spawn the `seo-google` agent to enrich the audit with real Google field data: CrUX Core Web Vitals (replaces lab-only estimates), GSC URL indexation status, search performance (clicks, impressions, CTR).
The Performance (CWV) category score benefits most from field data.

## Analytics Integration (Optional)

If the Seline MCP server is connected (`mcp__seline__*` tools present), pull
traffic evidence yourself with `seline_get_visit_metrics` and
`seline_get_pages` (`type: "entry"`), 30 days by default, and read
`seo-analytics` for the organic referrer filter and its caveats. Seline is the
only analytics source in this install: never ask for a GA4 property.
