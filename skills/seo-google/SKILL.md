...
description: >
  ...
  search performance. Site traffic lives in `seo-analytics` (Seline), not here.
  Use when user says "search console", "GSC", "PageSpeed", "CrUX", "field data",
...
Chrome user metrics, real indexation status and search performance. On-site
traffic and behaviour come from `seo-analytics` (Seline MCP), never from GA4.
...
| **3** (Ads) | + `ads_developer_token` + `ads_customer_id` | Tier 1 + `keywords`, `volume` |

Tier 2 was GA4 traffic and no longer exists. Traffic comes from
`seo-analytics` (Seline MCP), with no Google credential involved.
...
Traffic, sessions, landing pages and bounce rate come from `seo-analytics`
(Seline MCP). There is no GA4 command here and no GA4 property to configure.
Search Console gives you clicks and impressions on the SERP; Seline gives you
what happened after the click. Report both, never merge them.
...
- **seo-analytics**: Seline holds all on-site traffic. Pair GSC clicks with Seline sessions; never substitute one for the other
