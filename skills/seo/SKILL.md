...
| `/seo google [command] [url]` | Google SEO APIs (GSC, PageSpeed, CrUX, Indexing) |
| `/seo analytics [command] [period]` | Site analytics via Seline MCP (visits, pages, referrers, events) |
...
16. **seo-google** -- Google SEO APIs (GSC, PageSpeed, CrUX, Indexing API)
...
25. **seo-analytics** -- Site analytics via Seline MCP. The only analytics source in this install; GA4 is not used
...
installed: firecrawl, dataforseo, and image-gen, plus `/seo bing`,
`/seo profound`, and `/seo unlighthouse`. Ahrefs and SE Ranking are not
installed here: route every keyword, SERP, backlink and rank-tracking request
to `seo-dataforseo` (DataForSEO MCP) instead.
